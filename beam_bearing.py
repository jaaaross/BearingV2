...
'''
I think what would be best is to allow the user the option to load in a CSV or Excel or json file with all the parameters of all the different connections
they're interested in.
Design Code - will probably just do the most basic but will be good to call it out
Beam 1 Parameters (Width, Depth, DL, LL)
Beam 2 Parameters (Width, Depth, DL, LL)
Column Parameters (Width, Depth)
Other Parameters (F_c_perp, Fire Resistance Rating)

This all could be split over floors and columns similar to the pandas 2 workbook stuff
Then the program could calculate minimum bearing length to achieve 100%, 95%, 90% depending on input.
Would then output:
Beam 1 Parameters (Route length, route width, effective bearing length)
Beam 2 Parameters (Route length, route width)
